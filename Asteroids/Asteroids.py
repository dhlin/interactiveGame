...
def draw(canvas):
    global time, started, my_ship, rock_group, missile_group, explosion_group, lives, score
...
def rock_spawner():
    ...
    rock_ship_dist = dist(a_rock.get_position(), my_ship.get_position())
    rock_ship_radius = a_rock.get_radius() + my_ship.get_radius() + 20
    if len(rock_group) < 12 and rock_ship_dist > rock_ship_radius: 
        rock_group.add(a_rock)
        
def process_sprite_group(group_set, canvas):
    for sprite in group_set.copy():
        sprite.draw(canvas)
        if sprite.update():
...
def group_collide(group_set, other_object):  
    ...
    for sprite in set(group_set):
        if sprite.collide(other_object):
            explosion = Sprite(sprite.get_position(), [0, 0], 0, 0, 
...
def group_group_collide(group_set, group_otherobject):
    collide_number = 0
    for missile in set(group_otherobject):   
        if group_collide(group_set, missile):
            group_otherobject.remove(missile)
            collide_number += 1
